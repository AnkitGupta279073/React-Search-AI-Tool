const RecentSearch = ({ recentHistroy, showSidebar, onSelectHistory, onClearHistory, onDeleteItem }) => {
    return (
        <div className={`${showSidebar ? 'translate-x-0' : '-translate-x-full'
            ...
            <h1 className='text-white text-2xl flex text-center justify-center border-b border-zinc-700 px-2'>
                ...
                <button onClick={onClearHistory} className='cursor-pointer' title='Clear all history'>
            ...
            <ul className='text-left overflow-auto mt-2 h-[calc(100vh-80px)]'>
                {
                    recentHistroy.length === 0 ? (
                        <p className='text-zinc-400 text-sm text-center mt-2'>No recent search history</p>
                    ) : (
                        recentHistroy && recentHistroy.map((item, index) => (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className='ml-2 hover:bg-zinc-500 rounded p-1 transition-colors cursor-pointer'
